...
// 图标系统模块 (Icons Module)
// ============================================
// 本模块提供基于 Nerdfont 字体的图标系统，替代原有的 SVG 图标实现
// 使用 Unicode 字符作为图标，支持主题色跟随和样式自定义

#import "config.typ": fonts, theme-color
...
/// 示例:
///   #icon("phone")  // 渲染电话图标
///   #icon("github", color: blue)  // 渲染蓝色 GitHub 图标
#let icon(
  name,
  color: theme-color,
  size: 1em,
...
#let icon-group(
  ...
  names.map(name => icon(name, color: color)).join(h(gap))
}
...
#let tech-icon(
  ...
  if tech-lower in icon-chars {
    icon-label(icon-name, tech, color: color)
  } else {
    // 否则只显示技术名
    text(fill: color, tech)
  }
}
...
#let icon-circle(
  ...
  box(
    ...
    radius: 50%,
    fill: bg-color,
    align(center + horizon, icon(name, color: fg-color, size: size * 0.6))
  )
}
